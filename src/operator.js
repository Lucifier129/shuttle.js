  ...
  let callback = guard((type, payload) => {
    if (type === START) {
      callbackList = sourceList.map(
        source => source |> onNext(next) |> onFinish(finish) |> start
      )
      sink(START)
    } else if (type === NEXT) {
      sink(ASYNC)
...
export const combine = (...sourceList) => sink => {
  ...
  let callbackList = sourceList.map((source, index) => {
    ...
    return (
      source
      |> onNext(payload => next(payload, index))
      |> onFinish(finish)
      |> toCallback
    )
  })
...
export const switchMap = makeSource => source => sink => {
  ...
  let outerCallback = source((type, payload) => {
    if (type === NEXT) {
      ...
      innerCallback =
        makeSource(payload) |> onNext(next) |> onFinish(finish) |> toCallback
      innerFinished = false
      innerCallback(START)
    } else if (type === FINISH) {
      outerFinished = true
