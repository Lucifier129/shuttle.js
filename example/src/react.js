...
import { merge, scan, map, startWith, fromShape, consume, share, concat, then, take, keep } from 'sukkula/src/operator'
import { run } from 'sukkula/src/sink'
...
export const renderTo = container => source => source |> run(vdom => ReactDOM.render(vdom, container))
...
function createStore(reducers, preloadState) {
	...
	let reducers$ = actionTypeList.map(key => fromEvent(emitter, key) |> map(reducers[key]))
	let state$ =
		merge(...reducers$) |> startWith(identity) |> scan((state, reducer) => reducer(state), preloadState) |> share(true)
	return {
		state$: state$,
...
let toComponent = render => source => source |> switchMap(render) |> map(value => () => value)
...
let Header$ = store.state$ |> map(Math.abs) |> map(count => count % 5) |> map(n => `h${n + 1}`)
let Incre$ =
	interval(1000) |> take(1) |> toComponent(() => <button onClick={of(() => store.action.incre(1))}>+1</button>)
...
let app = (
	<div>
		...
		<Incre$ />
		<button onClick={of(() => store.action.decre(1))}>-1</button>
	</div>
)
