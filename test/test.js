...
test('operator: keep with start', done => {
...
test('operator: switchMap in async & sync', done => {
...
test('operator: switchMap in both sync', done => {
...
test('operator: switchMap in sync & async', done => {
  let list = [1, 2, 3]
  fromArray([1, 2, 3])
    |> onNext(n => expect(n).toBe(list.shift()))
...
test('operator: switchMap in one value', done => {
  let list = [2, 3, 4]
  fromArray([1])
    |> switchMap(n => fromArray([n + 1, n + 2, n + 3]))
    |> onNext(n => expect(n).toBe(list.shift()))
...
test('operator: fromShape in flat', done => {
  let shape = {
    a: interval(10),
    b: interval(35)
  }
...
test('operator: fromShape in nest', done => {
  let shape = {
    array: [
      of(10),
      {
